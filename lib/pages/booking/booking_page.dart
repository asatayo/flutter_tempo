...
class _BookPageState extends State<BookingPage> {
  ...
  _updateStatus(String val, bool isTodate) {
    setState(() {
      ...
      _comparisonData = widget.isHouse
          ? int.parse(widget.houseModel!.days)
          : int.parse(widget.roomModel!.hours);

      if (_hoursOrDays < _comparisonData) {
        String daysOrHours = widget.isHouse ? 'days_jm'.tr : 'hours_jm'.tr;
        String minimum =
            widget.isHouse ? "minimum_days".tr : "minimum_hours".tr;
        String message = minimum + _comparisonData.toString() + daysOrHours;
